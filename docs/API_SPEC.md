...
> **Search (UC-02):** Semantic search across document content (PRODUCT_SPEC UC-02) is handled via `POST /api/chat` with an appropriate query. A dedicated search endpoint that returns matching chunks without generating a full AI response is not included in MVP — `POST /api/chat` covers the P0 search use case. A distinct `GET /api/documents/{document_id}/search?q=...` may be added as a P1 enhancement.
...
## 13. Document Viewer URL

### `GET /api/documents/{document_id}/view-url`

| Field | Value |
|-------|-------|
| **Purpose** | Generate a short-lived signed URL for the frontend document viewer to display the file |
| **Auth** | Required (must own document) |
| **Response 200** | `{ "signed_url": "string", "expires_at": "datetime" }` |
| **Error 404** | Document not found or not owned by user |
| **Error 400** | Document is not in `ready` status |

**Note:** The `file_path` stored in the database is never returned to the client directly. The signed URL is generated by the backend using the Supabase Storage admin client and has a short expiry (e.g., 60–300 seconds). The frontend refreshes it as needed.
